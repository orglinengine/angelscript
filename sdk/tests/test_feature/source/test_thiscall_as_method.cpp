...
namespace TestThisCallMethod
{
	...
	Foo *Factory() { return new Foo(); }

	class RefCounter
	{
	public:
		RefCounter() : refCount(1) {}
		virtual void AddRef(Foo *f)
		{
			refCount++;
		}
		virtual void Release(Foo *f)
		{ 
			if( --refCount == 0 )
				delete f;
		}
	...
	bool Test()
	{
		...
		// Test asCALL_THISCALL_OBJLAST and asCALL_THISCALL_OBJFIRST with behaviors (since these calls do not go through the assembler code)
		// Reported by HenryAWE
		{
			...
			r = engine->RegisterObjectBehaviour("foo", asBEHAVE_FACTORY, "foo @f()", asFUNCTION(Factory), asCALL_CDECL, &refCounter);
			r = engine->RegisterObjectBehaviour("foo", asBEHAVE_ADDREF, "void f()", asMETHOD(RefCounter, AddRef), asCALL_THISCALL_OBJLAST, &refCounter);
			r = engine->RegisterObjectBehaviour("foo", asBEHAVE_RELEASE, "void f()", asMETHOD(RefCounter, Release), asCALL_THISCALL_OBJFIRST, &refCounter);

			r = ExecuteString(engine, "foo @f = foo(); foo @g = f;");
		...
		// Test asCALL_THISCALL_OBJLAST and asCALL_THISCALL_OBJFIRST
		{
